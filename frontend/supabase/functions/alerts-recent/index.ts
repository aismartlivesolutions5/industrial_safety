...
serve(async (req) => {
  ...
  try {
    ...
    const windowMinutes = url.searchParams.get("window_minutes") || "60";
    const topN = url.searchParams.get("top_n") || "20";

    if (isNaN(Number(windowMinutes)) || isNaN(Number(topN))) {
    ...
    const response = await fetch(
      `${BACKEND_URL}/alerts/recent?window_minutes=${windowMinutes}&top_n=${topN}`,
    ...
    console.error("Alerts recent error:", error);
